...
public class MotionProfileGenerator {
    ...
    public static void generate(MotionProfile profile, Waypoint[] waypoints, double timestep){
        Path path = new Path(waypoints);
        List<Pose2dCurved> states = path.split(0.01);
    ...
    public static MotionProfile generate(DriveSpecs drive, Pose2d start,
            Waypoint[] waypoints, double timestep){

        MotionProfile profile = new MotionProfile(drive, start);
        generate(profile, waypoints, timestep);
        return profile;
    }
